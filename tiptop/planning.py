...
def run_planning(
    ...
    skeleton was refused before cuTAMP ran; ``supplied_plan_failure`` is why cuTAMP could not solve
    one it accepted, held separately because a successful fallback search clears the returned
    failure_reason and would otherwise erase it.
    ...
    if reuse_plan_skeleton is not None:
        ...
        if rejection:
            _log.info(f"The given task plan does not solve this problem: {rejection}")
    ...
    if plan_out is not None:
        ...
        # Why the plan we were handed was refused before cuTAMP ran, for the caller's audit trail.
        # None when it was accepted (or when there was nothing to accept). Only logged before this.
        plan_out["rejection"] = rejection
        # ... and why cuTAMP itself could not solve it, when it was accepted and then failed. Held
        # separately from the returned failure_reason, which a successful fallback search clears.
        plan_out["supplied_plan_failure"] = supplied_failure
